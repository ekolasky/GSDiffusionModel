...
def add_colmap_to_category_folders(path):
    ...
    for sequence_name, frame_annotations in sequence_frame_annotations.items():
        # Get the image path
        img_path = os.path.join(path, sequence_name)
        ...
        for frame_annotation in frame_annotations:
            ...
            camera_data = {
                ...
                "focal_length": frame_annotation.viewpoint.focal_length,
            ...
            image_quaternion = _convert_rotation_to_quaternion(frame_annotation.viewpoint.rotation)
            image_data = {
                ...
                "tx": frame_annotation.viewpoint.translation[0],
                "ty": frame_annotation.viewpoint.translation[1],
                "tz": frame_annotation.viewpoint.translation[2],
        ...
        points3D_txt = _convert_ply_to_points3D(os.path.join(path, "points3D.ply"))

        with open(os.path.join(path, "camera.bin"), "w") as f:
            f.write(camera_txt)

        with open(os.path.join(path, "image.txt"), "w") as f:
            f.write(image_txt)
